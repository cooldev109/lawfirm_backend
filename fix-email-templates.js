// Script to fix email_templates table
...
async function fixEmailTemplates() {
  ...
  try {
    ...
    if (tableCheck[0].exists) {
      ...
      const hasTemplateKey = columns.some(c => c.column_name === 'template_key');

      if (!hasTemplateKey) {
        console.log('\nDropping old email_templates table and recreating...');
    ...
    const { rows: existing } = await client.query('SELECT COUNT(*) FROM email_templates');
    ...
    if (parseInt(existing[0].count) === 0) {
      ...
      await client.query(`
        ...
        (
          ...
          <p style="color: #718096; font-size: 14px;">If you have already taken action or have questions, please disregard this reminder or contact your attorney directly.</p>
          <p style="margin-top: 24px;">Best regards,<br/>{{firmName}}</p>
        </div>',
      ...
      console.log(`\n${existing[0].count} templates already exist`);
    }

    // Create indexes
    console.log('\nCreating indexes...');
    await client.query('CREATE INDEX IF NOT EXISTS idx_email_templates_key ON email_templates(template_key)');
    await client.query('CREATE INDEX IF NOT EXISTS idx_email_templates_active ON email_templates(is_active)');
    console.log('✓ Indexes created');

    // Verify
    const { rows: templates } = await client.query('SELECT template_key, name FROM email_templates ORDER BY name');
    console.log('\nEmail templates in database:');
    templates.forEach(t => console.log(`  - ${t.template_key}: ${t.name}`));
...
fixEmailTemplates().catch(() => process.exit(1));
